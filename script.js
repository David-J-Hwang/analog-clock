...
modeSwitch.addEventListener("click", () => {
  ...
  modeSwitch.textContent = isDarkMode ? "Turn to Light Mode" : "Turn to Dark Mode";
...
const updateTime = () => {
  ...
  minuteDeg.textContent = `분침 회전각: ${Math.floor(smoothMinToDeg * 10) / 10}`;
  secondDeg.textContent = `초침 회전각: ${Math.floor(smoothSecToDeg)}`;
  fullTime.textContent = `현재시각: ${date.getFullYear()}년 ${date.getMonth()}월 ${date.getDate()}일 ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
}
